import type CommandPaletteTypes from '#build/pohon/command-palette';
import type { DeepPartial } from '@vinicunca/perkakas';

// @unocss-include

export const commandPaletteTheme: DeepPartial<typeof CommandPaletteTypes> = {
  slots: {
  ...
  variants: {
    active: {
      ...
      false: {
        item: 'text-(--ui-text) data-highlighted:text-(--ui-text-highlighted) data-highlighted:before:bg-(--ui-bg-elevated)/50 transition-colors before:transition-colors',
        itemLeadingIcon: 'text-(--ui-text-dimmed) group-data-highlighted:text-(--ui-text) transition-colors',
      },
    },
    loading: {
      true: {
