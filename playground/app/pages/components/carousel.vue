  ...
  <div class="space-y-11">
    ...
    <template v-if="classNames">
      <UCarousel
        ...
        v-bind="bind"
        :items="items"
        :ui="{ item: 'basis-[70%] transition-opacity ease-in-out [&:not(.is-snapped)]:opacity-10', container: 'h-[352px]' }"
    ...
    <template v-else-if="autoHeight">
      ...
      >
        <img
          :src="`https://picsum.photos/600/${index % 2 === 0 ? 350 : 450}?v=${index}`"
          :class="index % 2 === 0 ? 'h-[350px]' : 'h-[450px]'"
          class="rounded-lg"
        >
      </UCarousel>
    </template>
    <template v-else>
      ...
      <template v-if="orientation === 'horizontal'">
        ...
        >
          <img
            :src="`https://picsum.photos/320/320?v=${index}`"
            class="rounded-lg"
          >
        </UCarousel>
      </template>
